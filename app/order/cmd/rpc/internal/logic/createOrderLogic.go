...
func (l *CreateOrderLogic) CreateOrder(in *rpc.CreateOrderReq) (*rpc.CreateOrderResp, error) {
	...
	// 优化，不再本地创建，而是直接构建消息推送到Kafka
	// 3. 【削峰改造】：构建 MQ 消息并推送到 Kafka
	expireTime := time.Now().Add(define.ExpireDuration)
	msg := &order_msg.CreateOrderMsg{
		OrderSn:    orderSn,
		UserId:     in.UserId,
		EventId:    in.EventId,
		SeatId:     seatInfo.Id,
	...
	// 假设在 ServiceContext 中已经初始化了 Kafka Pusher (kq.Pusher)
	// 此时不会阻塞等待 MySQL 执行，极大提高了接口吞吐量
	err = l.svcCtx.MQProducer.Send(l.ctx, "order_create", msg)
	if err != nil {
		l.Logger.Errorf("Failed to push create order message to Kafka: %v", err)
		// 如果 Kafka 投递失败（极少发生），此时 Redis 已锁座。
		// TODO 投递失败，该怎么办
		return nil, status.Error(codes.Internal, "系统繁忙，请稍后再试")
	}
